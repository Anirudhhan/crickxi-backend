...
func CreateBallEvent(tx *sqlx.Tx, delivery models.Delivery) error {
	...
	_, err := tx.Exec(query, delivery.InningsID, delivery.BallSequence, delivery.OverNumber, delivery.BallInOver,
		...
		delivery.RunsBatter, delivery.RunsExtra, delivery.ExtraType, delivery.IsWicket, delivery.WicketType, delivery.WicketPlayerID, delivery.FielderID, delivery.NextBatterID)
...
func GetLastBall(matchID string) (ball models.Delivery, err error) {
	query := `SELECT 
                ...
                b.wicket_player_id, b.fielder_id, b.next_batter_id
              FROM balls b
